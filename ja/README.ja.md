...
Note PilotはObsidianにAI作業パネルを追加します。現在のノートや選択テキストについてモデルに質問でき、書き込みリクエストをMarkdownノートの作成・追加・変更などのレビュー可能なVaultアクションに変換します。
...
Note Pilotは次の接続方式をサポートします：

| 接続 | 使用方式 |
| --- | --- |
| ChatGPTサブスクリプション | `http://127.0.0.1:10531/v1`のローカル`openai-oauth`プロキシ |
| OpenAI | OpenAI互換`/chat/completions` API |
| Anthropic Claude | Anthropic Messages API |
| OpenRouter | OpenAI互換`/chat/completions` API |
| Groq | OpenAI互換`/chat/completions` API |
| Gemini API | GeminiのOpenAI互換エンドポイント |
| DeepSeek | OpenAI互換`/chat/completions` API |
| Ollama / local | ローカルOpenAI互換エンドポイント |
| Custom OpenAI-compatible endpoint | 互換性のある任意のbase URLとモデル |

## これらの機能が存在する理由

Note Pilotは一つの原則を中心に設計されています：AIは変更を提案できますが、Vaultの制御権はユーザーにあります。
...
graph TD
    ...
    Provider -->|"APIキープリセット"| Compatible[OpenAI, OpenRouter, Groq, Gemini, DeepSeek, Ollama, Custom]
...
Obsidianを再起動し、設定 → コミュニティプラグインから`Note Pilot`を有効にします。

## プロバイダー設定

設定 → コミュニティプラグイン → Note Pilotで接続モードを選択します。
...
`API key provider`を選択してから、次のAPIキープリセットのいずれかを選びます：
...
ホスト型プロバイダーではAPIキーを入力してから`Test connection`を使用します。Ollama / localではAPIキーを空欄にし、ローカルサーバーが起動していることを確認してからテストします。
...
Note Pilotは設定に応じてノートコンテンツを外部サービスに送信することがあります。
