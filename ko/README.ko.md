...
Note Pilot는 Obsidian에 AI 작업 패널을 추가합니다. 현재 노트나 선택한 텍스트에 대해 모델에 질문할 수 있으며, 작성 요청을 마크다운 노트의 생성·추가·수정 등 검토 가능한 볼트 액션으로 변환합니다.
...
Note Pilot는 다음 연결 방식을 지원합니다:

| 연결 | 사용 방식 |
| --- | --- |
| ChatGPT 구독 | `http://127.0.0.1:10531/v1`의 로컬 `openai-oauth` 프록시 |
| OpenAI | OpenAI 호환 `/chat/completions` API |
| Anthropic Claude | Anthropic Messages API |
| OpenRouter | OpenAI 호환 `/chat/completions` API |
| Groq | OpenAI 호환 `/chat/completions` API |
| Gemini API | Gemini의 OpenAI 호환 엔드포인트 |
| DeepSeek | OpenAI 호환 `/chat/completions` API |
| Ollama / local | 로컬 OpenAI 호환 엔드포인트 |
| Custom OpenAI-compatible endpoint | 호환 가능한 임의의 base URL과 모델 |

## 이러한 기능이 존재하는 이유

Note Pilot는 하나의 원칙을 중심으로 설계되었습니다: AI는 변경을 제안할 수 있지만, 볼트의 통제권은 사용자에게 있습니다.
...
graph TD
    ...
    Provider -->|"API 키 프리셋"| Compatible[OpenAI, OpenRouter, Groq, Gemini, DeepSeek, Ollama, Custom]
...
Obsidian을 재시작한 후 설정 -> 커뮤니티 플러그인에서 `Note Pilot`를 활성화합니다.

## 프로바이더 설정

설정 -> 커뮤니티 플러그인 -> Note Pilot에서 연결 모드를 선택합니다.
...
`API key provider`를 선택한 뒤 다음 API 키 프리셋 중 하나를 고릅니다:
...
호스팅 프로바이더는 API 키를 입력한 후 `Test connection`을 사용합니다. Ollama / local은 API 키를 비워두고 로컬 서버가 실행 중인지 확인한 뒤 테스트합니다.
...
Note Pilot는 설정에 따라 노트 내용을 외부 서비스로 전송할 수 있습니다.
